...
@interface MUITableViewController () <UIViewControllerRestoration>
...
//    for(UIViewController *vc in self.childViewControllers){
      //  [self.childTableViewController setEditing:editing animated:animated];
//    }
  //  if(animated){
...
// TODO its nil when entering edit
- (id)currentDetailModelIdentifier{
    return self.mui_splitViewController.mui_currentDetailModelIdentifier;
}
    ...
    if(splitItem){
        if(!self.isEditing){
            ...
            if([self.tableView.dataSource conformsToProtocol:@protocol(UIDataSourceModelAssociation)]){
                ...
                if([rows.firstObject compare:indexPath] == NSOrderedDescending){
                    sp = UITableViewScrollPositionTop;
                }
                else if([rows.lastObject compare:indexPath] == NSOrderedAscending){
...
//    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(didShowDetailViewController:) name:MUIDetailViewManagerWillShowDetailViewControllerNotification object:detailViewManager];
    ...
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(currentDetailModelIdentifierDidChangeNotification:) name:UIViewControllerCurrentDetailModelIdentifierDidChangeNotification object:nil];
//    [self updateSelectedObject];
//    [self updateRowSelection];
}
